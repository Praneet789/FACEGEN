...
export function warpImageToCanvas(image, canvas, rows, cols, points // normalized [0..1]
) {
    ...
    const getSrc = (r, c) => ({ x: (c / (cols - 1)) * w, y: (r / (rows - 1)) * h });
    const getDst = (r, c) => {
    ...
    for (let r = 0; r < rows - 1; r++) {
        for (let c = 0; c < cols - 1; c++) {
            ...
            const srcA = [getSrc(r, c), getSrc(r, c + 1), getSrc(r + 1, c)];
            const dstA = [getDst(r, c), getDst(r, c + 1), getDst(r + 1, c)];
            ctx.save();
            ctx.beginPath();
            ...
            ctx.restore();
            const srcB = [getSrc(r + 1, c), getSrc(r, c + 1), getSrc(r + 1, c + 1)];
            const dstB = [getDst(r + 1, c), getDst(r, c + 1), getDst(r + 1, c + 1)];
            ctx.save();
            ctx.beginPath();
