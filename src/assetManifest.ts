import { AssetDefinition, AssetCategory } from './types';

// Helper to build a numbered sequence only (no group variants)
const seq = (opts: { prefix: string; folder: string; category: AssetCategory; count: number; pad?: number; label?: string }) => {
...
const accessories: AssetDefinition[] = seq({ prefix: 'more', folder: 'more', category: 'accessories', count: 6, label: 'More' });

export const assetManifest: Record<AssetCategory, AssetDefinition[]> = {
  eyes,
  noses,
  mouths,
  eyebrows,
