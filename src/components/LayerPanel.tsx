...
export const LayerPanel: React.FC = () => {
  ...
  return (
    <aside className="w-64 shrink-0 backdrop-blur-md border-l border-white/40 dark:border-gray-700 overflow-y-auto bg-white/60 dark:bg-gray-900/40 px-3 py-4">
      ...
      <ul className="space-y-1">
        {placed.slice().sort((a,b)=> b.zIndex - a.zIndex).map(p => {
          ...
          return (
            <li key={p.instanceId} onClick={() => select(p.instanceId)} className={`group flex items-center gap-2 text-[11px] px-2 py-1.5 rounded-md cursor-pointer border transition ${active ? 'bg-blue-600/90 border-blue-500 text-white shadow-sm' : 'bg-white/70 dark:bg-gray-800/60 border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 hover:border-blue-400 hover:bg-blue-50/70 dark:hover:bg-gray-700/60'}`}>
